...
const Simulation = () => {
    ...
    const taxableIncomeByYear = useMemo(() => {
        ...
        const municipalTaxFactor = percentToFactor(municipalTaxRate, scenarioDetails?.municipal_tax_factor ?? 1.15);
        const cantonalTaxFactor = percentToFactor(cantonalTaxRate, scenarioDetails?.cantonal_tax_factor ?? 0.98);
        const churchTaxFactor = percentToFactor(churchTaxRate, scenarioDetails?.church_tax_factor ?? 0.14);
